(function () {
  ...
  // cards) whenever there's a saved note for them - clicking it opens the same reason popup
  // the canvas itself uses, so the note is reachable without having to dig them out of a folder
  ...
  function scan() {
    for (const namebox of document.querySelectorAll(NAMEBOXSEL)) {
      ...
      badge.style.cssText = "display:inline-flex;vertical-align:middle;margin-left:2px;cursor:pointer;width:13px;height:13px";
