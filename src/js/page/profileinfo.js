(function () {
  ...
  // twitter's own OUTLINE location pin (lifted from its about-panel "based in" row) - matches the
  // real UserLocation icon, unlike a filled material pin
  ...
  function itemgray(items) {
    // must be a GRAY item - NOT UserUrl, which is the blue link colour. a combined selector would
    // return whichever is first in DOM order (UserUrl, on a profile with no user-set location), so
    // pick the join date (always present + gray) explicitly, then location
